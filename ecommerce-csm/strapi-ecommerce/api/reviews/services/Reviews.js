...
module.exports = {
  ...
  add: async (values) => {
    ...
    return Reviews.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  /**
   * Promise to search a/an reviews.
   *
   * @return {Promise}
   */
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Reviews.attributes).reduce((acc, curr) => {
      switch (Reviews.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
